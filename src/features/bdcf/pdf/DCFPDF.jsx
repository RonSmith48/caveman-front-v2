...
const getStatusStyle = (status) => {
  switch (status.toLowerCase()) {
    case 'charged':
      return pdfStyles.statusSuccess;
    case 'drilled':
      return pdfStyles.statusWarning;
    case 'fired':
      return pdfStyles.statusSecondary;
    default:
      return null;
  }
};
  ...
  <Page size="A4" orientation="portrait" style={pdfStyles.page}>
    <View style={pdfStyles.headerRow}>
      <View style={pdfStyles.titleContainer}>
        <Text style={pdfStyles.title}>DCF Report</Text>
        <Text style={pdfStyles.subTitle}>for {formatDate(reportDate)}</Text>
      </View>
    ...
    {data.map((ring, index) => (
      <View key={index} style={pdfStyles.row}>
        ...
        <Text style={[pdfStyles.cellStatus, getStatusStyle(ring.activity)]}>{ring.activity}</Text>
      </View>
    ))}
